...
Para poder armar este laboratorio se requiere previamente de un **visualizador** y conexión a internet. El virtualizador se Utilizara principalmente para el servidor y para virtualizar un entorno seguro donde atacar un OS que se encuentre conectado al servidor WAZUH.
...
### WAZUH-SERVER
El Sistema SIEM que se utiliza para este laboratorio se puede instalar de dos formas. La primera es la forma manual donde se instalan todas las dependencias de forma independiente, y la segunda que el la mas sencilla y la que se utiliza en esta guía que es utilizando el **quickstart** de wazuh.
...
```
Con este comando se empieza con la instalacion de WAZUH-SERVER. Una vez finalizado es necesario ingresar en el dashboard para verificar que se instalo correctamente. se ingresa en `https://<TU-IP>:443` con las siguentes credenciales:</br>
User: `admin`</br>
...
Teniendo el servidor en funcionamiento es necesario ingresar los `agentes`, estos se pueden instalar mediante una linea de codigo que genera el propio sistema en `Menu => Agents Management => Sumary => Deploy new Agent`.

Con estos se puede empezar a visualizar los logs y alertas que generen los agentes dentro del dashbard.
...
Logstash Indexa logs para integrarlos en el servidor y poder visualizarlos, estos logs son enviados de algunas herramientas como los `Beats` que se encuentran en los endpoints.

Para que Logstash pueda encontrar los logs que se le manda se debe crear un archivo de configuracion donde indique el puerto que tiene que escuchar y el archivo que tiene que buscar.</br>
...
Con el comando `ssl => true` y `ssl_certificate_verification => false` se permite que el log ingrese por un canal seguro sin que necesite verificacion, con esto se puede ahorrar un paso.

> [!IMPORTANT]
> Todas estas especificaciones se hacen dentro de un entorno de prueba para practicar dentro de un sistema SIEM, de ser posible cuanto mas seguridad tenga el servidor para todos los archivos que integre mejor.

Logstash tambien necesita los plugins con los cuales detecta los `Outputs`, y como WAZUH esta basado en `OpenSearch` se debe instalar el plugin con.
...
Primero se debe descargar el archivo `.zip` desde la pagina oficial de [Winlogbeat](https://www.elastic.co/es/downloads/beats/winlogbeat) o [Filebeat](https://www.elastic.co/es/downloads/beats/filebeat). La diferencia entre estos se encuentra en que **Winlogbeat** captura logs unicamente de windows y **Filebeat** permite capturar logs desde cualquier Sistema operativo. Para el ejemplo se va a explicar la instalacion de Winlogbeat, aun asi la instalación funciona casi igual para ambos.</br>
...
Para instalar winlogbeat se debe ejecutar el archivo `install-service-winlogbeat.ps1`, este archivo se debe ejecutar dentro de powershell en modo **Administrador**, dentro de Powershell deberia verse asi.
...
Pasado un pequeño lapso de tiempo se deberia poder ver los logs en "`Menu => Indexer Management => Index Management`".
...
Primero se agrega el repositorio a APT y se instala Suricata.
...
Con la configuración finalizada, se debe reiniciar utilizando `sudo systemctl restart wazuh-agent`. Con esto Wazuh-Server deberia estar recibiendo los logs de suricata.

> [!TIP]
> Una forma de verificar que Suricata funciona es utilizar el comando `ping -c 5 <LINUXorWIN_IP>` y con esto se deberia poder observar los logs de la siguiente forma.
> ![image](/Capturas/Suricata_Prueba.png)

### Herramienta de ataque y Simulación

Para poder poner a prueba el laboratorio es necesario utilizar una herramienta de **ataques simulados**, cada sistema operativo tiene sus propias herramientas. Para linux se utiliza el framework **Metasploit** y para Windows se utiliza **Atomic-Red-Team**. Cada herramienta tiene una pequeña guia de instalacion en la carpeta [Attack-Tool](/Attack-Tool).</br>

Con estas herramientas se pueden ejecutar los siguientes comandos, como por ejemplo para Atomic-Red-Team.
...
```shell
 msfconsole -q -x "use auxiliary/scanner/http/http_version; set RHOSTS <IP_OBJETIVO_LOCAL>; set RPORT 80; run; exit"
```
